<template>
  <div id="test">
    ...
    <p style="white-space: pre">{{ message2 }}</p>    <!-- 空格会被保留 -->
    <textarea v-model="message2" placeholder="多行文本输入……"></textarea><br>
    ...
    <!-- v-on：用于DOM事件监听 -->
    <button @click="reverseMessage">反转字符串</button><br>
    ...
    <ul>
      <li v-for="(key, index) in sites" :key="index">
        {{ key }} : {{ index }}
      </li>
    </ul>
    ...
    <div id="app">
      ...
      <div v-bind:style="[baseStyles, overridingStyles]">菜鸟教程</div><br>
    </div>
    ...
    <br>
    <span>选择的值为: {{ checkedNames }}</span><br><br>
    ...
    <div v-runoob="{ color: 'green', text: '菜鸟教程!' }"></div><br>
    ...
    <!-- 在js中页面跳转 -->
    <button @click = "jumpToE()">跳转</button><br><br>
    ...
    <button v-on:click = "clock">开启定时器</button><br>
  </div>
</template>
